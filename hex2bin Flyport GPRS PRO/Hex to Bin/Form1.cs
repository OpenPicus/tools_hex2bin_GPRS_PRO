        ...
        * ****************************************************************/
        ...
        private int Convert2bin(StreamReader filetoconv)
        {
            ...
            if (dlg_binfilesave.FileName != "")
            {
                ...
                FileStream file_bin_read = new FileStream(dlg_binfilesave.FileName, FileMode.Open, FileAccess.Read);
                int startPosition = 0;
                BinaryReader bin_reader = new BinaryReader(file_bin_read);
                byte[] filBinContent = bin_reader.ReadBytes(256512);
                // Split  file to multiple files to allow smarter FTP Downloads...
                tbOut.Text += "Split files in progress...";
                int splitBinLenFile = 4096 * 7; // New split file size 28672 (4096 multiple - flash page size!)
                int lastSplitBinLen = 256512 - (splitBinLenFile * 8); // 27136 bytes

                for (int s = 0; s < 9; s++)
                {
                    String fileNewName = dlg_binfilesave.FileName + "." + s.ToString();
                    if (fileNewName == null)
                    {
                        MessageBox.Show("Error in Slitted filename. Operation aborted!");
                        return -1;
                    }
                    Console.Write("Writing file: " + fileNewName);
                    FileStream file_bin_split = new FileStream(fileNewName, FileMode.Create, FileAccess.ReadWrite);
                    BinaryWriter stream_bin_split = new BinaryWriter(file_bin_split);

                    startPosition = splitBinLenFile * s;
                    if(s == 8) // Last file:
                    {
                        splitBinLenFile = lastSplitBinLen;
                    }

                    stream_bin_split.Write(filBinContent, startPosition, splitBinLenFile);
                    stream_bin_split.Close();

                    Console.WriteLine(" Done!");
                    Console.Write("Appending MD5...");

                    // Appending MD5 Calculation on file
                    string hashText = "";
                    string hexValue = "";
                    byte[] fileData = File.ReadAllBytes(fileNewName);
                    ...
                    FileStream file_md5 = new FileStream(fileNewName, FileMode.Append, FileAccess.Write);
